...
/////// Vectors:
                ...
                float3 lightColor = _LightColor0.rgb;
				float _CloudValue = 0.5;

////// Texture   
				float PI = 3.141592;
				
				float positionSet = i.uv.y;

		    	i.uv.y= i.uv.y*0.5 + 0.5;
				i.uv.x = (PI+atan2(i.posWorld.x,i.posWorld.z))/(2*PI);
				
				float3 normalizeWorldPostion = _WorldSpaceLightPos0.xyz*0.5+0.5;
	
				float4 _CloudTexture = tex2D(_CloudTex,TRANSFORM_TEX(float2(i.uv.x+_Time.y*0.0005, i.uv.y), _CloudTex));
				float4 _CloudTexture3 = tex2D(_CloudTex2,TRANSFORM_TEX(float2(i.uv.x+_Time.y*0.0035, i.uv.y), _CloudTex2));
				float4 _CloudTexture2 = tex2D(_CloudTex1,TRANSFORM_TEX(float2(i.uv.x+_Time.y*0.0015, i.uv.y), _CloudTex1));
				
			
				float3 _CloudTextureLerp2=lerp(_CloudTexture2.rgb* _CloudColor.rgb  ,  _CloudTexture2.rgb , normalizeWorldPostion.y);
				float3 _CloudTextureLerp=lerp(_CloudTexture3.rgb*_CloudColor.rgb  ,  _CloudTexture3.rgb  , normalizeWorldPostion.y);
				float3 _CloudTextureLerp3=lerp(_CloudTexture.rgb*_CloudColor.rgb  ,  _CloudTexture.rgb  , normalizeWorldPostion.y);
				
				
				
				
		//	float4 _CloudTexture = tex2D(_CloudTex,TRANSFORM_TEX(float2(i.uv.x, i.uv.y), _CloudTex));
				//float4 _CloudTexture2 = tex2D(_CloudTex1, TRANSFORM_TEX(float2(i.uv.x + _Time.y*0.005, i.uv.y), _CloudTex1));
				float4 _StarTex=tex2D(_StartTex, TRANSFORM_TEX(float2(i.uv.x + _Time.y*0.0005, i.uv.y), _StartTex));


////// Emissive:
                float _Min = min(_SunRadiusA,_SunRadiusB);
                float _OneMa = (1.0 - (_Min*_Min));
                float _Max = max(_SunRadiusA,_SunRadiusB);
                float _one = 1.0;


				
				
				float3 StartLerp=lerp(_StarTex.rgb, float3(0,0,0),normalizeWorldPostion.y );

//sky Color
				float3 Sunset3 = lerp(_SkyColor.rgb, _SunSetColor.rgb, normalizeWorldPostion.y);
				float3 SkyColor = lerp(Sunset3.rgb, _HorizonColor.rgb, pow((1.0 - max(0, dot(viewDirection, float3(0, -1, 0)))), 8.0));
				float3 FinalSkyColor = lerp(SkyColor, _SkyTopColor.rgb, max(0, dot(viewDirection, float3(0, -1, 0))));

//sky Color2 
				float3 Sunset = lerp(_SunSetColor.rgb, _SkyColor2.rgb, saturate(dot(float3(0,1,0),_WorldSpaceLightPos0.xyz)));
				float3 SkyColor2 = lerp(Sunset.rgb, _HorizonColor2.rgb, pow((1.0 - max(0, dot(viewDirection, float3(0, -1, 0)))), 8.0));
				float3 FinalSkyColor2 = lerp(SkyColor2, _SkyTopColor2.rgb, pow(max(0, dot(viewDirection, float3(0, -1, 0))),1));
			
			

//Sun
				float3 sun = (pow(saturate((_one + ((max(0, dot((-1 * lightDirection), viewDirection)) - _OneMa) * (0.0 - _one)) / 
				((1.0 - (_Max*_Max)) - _OneMa))), 5.0)*_SunIntensity);
				
				float3 emissive = lerp(FinalSkyColor.rgb, FinalSkyColor2.rgb,normalizeWorldPostion.y);
                float3 finalColor = emissive;


     
		    return fixed4(finalColor+_CloudTextureLerp2+_CloudTextureLerp3+_CloudTextureLerp+StartLerp.rgb+sun,0);
